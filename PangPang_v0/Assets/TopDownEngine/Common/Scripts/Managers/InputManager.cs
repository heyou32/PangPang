...
namespace MoreMountains.TopDownEngine
{
    ...
    public class InputManager : MMSingleton<InputManager>
    {
        ...
        [MMCondition("joystickMovementEnabled", true)]
...
#if UNITY_EDITOR
            // PC 테스트용 조이스틱
            JoysticMovement(xAxis);
            JoysticMovement(yAxis);
#endif
        }
        void JoysticMovement(float axis)
        {
            if (joystickMovementEnabled)
            {
                //joystickKnob.localPosition.smoo
                //joystickKnob.set.localPosition.x = Input.GetAxis(_axisHorizontal);
