...
namespace MoreMountains.TopDownEngine
{
    ...
    public class ExplodudesBomb : TopDownMonoBehaviour
    {
        ...
        protected virtual void Initialization()
        {
            ...
            _boxCollider = this.gameObject.GetComponentNoAlloc<BoxCollider>();
        ...
        protected virtual DamageOnTouch CreateDamageArea(string name)
        {
            ...
            damageAreaGameObject.name = "BombDamageArea" + name;
        ...
        /// <summary>
        /// 4개의 방향으로 DamageArea를 만들기 전에 막혀있는 곳을 확인합니다 
        /// </summary>
        protected virtual void CastRays()
        {
            ...
            _raycastEast = PhysicsDebug.Raycast3D(this.transform.position + Vector3.right * boxWidth + RaycastOffset, Vector3.right, MaximumRaycastDistance, ObstaclesMask, Color.red, true);
            if (_raycastEast.collider != null) { _obstacleEastDistance = _raycastEast.distance; } else { _obstacleEastDistance = 0f; }

            _raycastNorth = PhysicsDebug.Raycast3D(this.transform.position + Vector3.forward * boxWidth + RaycastOffset, Vector3.forward, MaximumRaycastDistance, ObstaclesMask, Color.red, true);
            if (_raycastNorth.collider != null) { _obstacleNorthDistance = _raycastNorth.distance; } else { _obstacleNorthDistance = 0f; }

            _raycastSouth = PhysicsDebug.Raycast3D(this.transform.position + Vector3.back * boxWidth + RaycastOffset, Vector3.back, MaximumRaycastDistance, ObstaclesMask, Color.red, true);
            if (_raycastSouth.collider != null) { _obstacleSouthDistance = _raycastSouth.distance; } else { _obstacleSouthDistance = 0f; }

            _raycastWest = PhysicsDebug.Raycast3D(this.transform.position + Vector3.left * boxWidth + RaycastOffset, Vector3.left, MaximumRaycastDistance, ObstaclesMask, Color.red, true);
